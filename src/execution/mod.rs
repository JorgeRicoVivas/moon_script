...
pub struct ASTFunction {
    pub(crate) function: fn(Vec<ReducedValue>) -> Result<ReducedValue, String>,
...
impl ExecutingContext {
    fn execute_block(&mut self, block: &Block) -> Result<Option<ReducedValue>, Vec<String>> {
        match block {
            Block::WhileBlock { condition, statements } => {
                while self.resolve_value(condition.clone())?.try_into().map_err(|_| vec!["Couldn't solve a while loop's condition".to_string()])? {
            ...
            Block::IfElseBlock { condition, positive_case_statements, negative_case_statements } => {
                if self.resolve_value(condition.clone())?.try_into().map_err(|_| vec!["Couldn't solve an if block's condition".to_string()])? {
            ...
            Block::FnCall(function) => {
                ...
                let args = function.args.iter().map(|value| self.resolve_value(value.clone()))
                    .on_errors(|error| errors.extend(error.into_iter()))
                    .collect();
                if !errors.is_empty() {
                    return Err(errors);
                }
                (function.function)(args).map_err(|error| vec![error])?;
            }
    ...
    fn resolve_value(&mut self, value: FullValue) -> Result<ReducedValue, Vec<String>> {
        Ok(match value {
            ...
            FullValue::Function(function) => {
                ...
                for value in function.args.into_iter().map(|value| self.resolve_value(value)) {
                    match value {
